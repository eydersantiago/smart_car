...
class CostoUniformeMixin:
    def busqueda_costo_uniforme(self, inicio, objetivo):
        ...
        return None, nodos_expandidos, self.profundidad, float('inf'), exploracion  # Si no se encuentra un camino
    ...
    def busqueda_costo_uniforme_total(self):
        ...
        if not camino1:
            return None, nodos1, profundidad1, float('inf'), exploracion1  # No encontró al pasajero
        ...
        if not camino2:
            return None, nodos1 + nodos2, max(profundidad1, profundidad2),float('inf'), exploracion1 + exploracion2  # No encontró el destino
